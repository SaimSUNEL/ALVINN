...
# Only difference is that the input is 3 gray images, 30000 input layer...
...
# Validation set, speed, direction angle are being loaded....
...
# Training set, speed, direction angle are being loaded....
...
for dosya in validation_image_file_list:
    if dosya.__contains__("GRAYResized"):
        ...
        image_file = cv2.imread("../DataCollector/"+location_name+"/" + str(validation_image_number) + "GRAYResized.png")
...
for dosya in image_file_list:
    if dosya.__contains__("GRAYResized"):
        ...
        image_file = cv2.imread("../DataCollector/"+location_name+"/" + str(image_number) + "GRAYResized.png")
        image_file = image_file[:, :, 0]
...
#Normalizing validation and training sets...
...
image_set = [np.reshape(whole_image_set[index], (10000))
             for index in range(0, whole_image_set.shape[0])]
validation_image_set = [np.reshape(validation_whole_image_set[index], (10000)) for index in range(0, validation_whole_image_set.shape[0])]
...
pickle.dump([direction_max, direction_min,
             ...
             whole_image_set.min(), diff], open("MLPRegression3ImageTrainer("+road_name+")_important_data.dat", "w"))
...
# Stacking 3 sequential validation set images into one...
...
for image_index in range(2, len(validation_image_set)):
    ...
    combinatination = np.reshape(combinatination, newshape=(30000))
    validation_new_image_set.append(combinatination)
    # Top image's corresponding angle and speed are cared...
...
# Stacking 3 sequential training set images into one...
...
error_graph_file_name = "MlpRegression3Images"+road_name+"_graph.dat"
...
if len([file for file in os.listdir("Trained_NN") if file.__contains__("MlpRegression3Images"+road_name)]) >0:
    train_saver.restore(sess, "Trained_NN/MlpRegression3Images"+road_name)
    # sess.run(init)
...
while iteration_count != 0:
    ...
    for iteration in range(iteration_count):
        ...
        if j < smallest_validation_error:
            ...
            train_saver.save(sess, "Trained_NN/MlpRegression3Images" + road_name)

        print "Loss : ", l, " Valid error : ", j
