...
validation_image_file_list = [ int(a[:-1*len("GRAYResized.png")]) for a in validation_image_file_list if a.__contains__("GRAYResized")]
...
for dosya in range(1, len(validation_image_file_list)):

    # current and previous images are packed...
    dosya_number = dosya - 1 # + 89
    dosya_number2 = dosya # +89

    image_file = cv2.imread("../DataCollector/"+location_name+"/"+str(dosya_number)+"GRAYResized.png")
    image_file = image_file[:, :, 0]

    image_file2 = cv2.imread("../DataCollector/"+location_name+"/"+str(dosya_number2)+"GRAYResized.png")
    ...
    validation_information_set.append(data_dictionary[str(dosya_number2)][1])# , int(data_dictionary[dosya_number][3])])
    validation_information_set2.append(data_dictionary[str(dosya_number2)][3])
...
image_file_list = [ int(a[:-1*len("GRAYResized.png")]) for a in image_file_list if a.__contains__("GRAYResized")]
# images should be ordered... because two consequence images will be packed...
image_file_list.sort()

print image_file_list

for dosya in range(1, len(image_file_list)):
    ...
    whole_image_set.append(combination)
...
indices = [a for a in range(len(image_file_list)-1)]
...
pickle.dump([direction_max, direction_min,
             ...
             whole_image_set.min(), diff], open("MLPClassification2ImagesTrainer("+road_name+")_important_data.dat","w"))
...
one_hot = tf.one_hot(target_values, data_size, on_value=1.0, off_value=0.0, dtype=tf.float32)
...
error_graph_file_name = "MLPClassification2Images"+road_name+"_graph.dat"
...
if len([file for file in os.listdir("Trained_NN") if file.__contains__("MLPClassification2Images"+road_name)]) >0:
...
train_saver.save(sess, "Trained_NN/MLPClassification2Images"+road_name)