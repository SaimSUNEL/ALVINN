...
# Only difference is that the input is 3 gray scale images, 100x100x3 input image...
...
image_set = [np.reshape(whole_image_set[index], (100, 100, 1))
...
pickle.dump([direction_max, direction_min,
             ...
             whole_image_set.min(), diff], open("CNNRegression3ImageTrainer("+road_name+")_important_data.dat", "w"))
...
for image_index in range(2, len(validation_image_set)):
    ...
    combinatination = np.reshape(combinatination, newshape=(100, 100, 3))
...
error_graph_file_name = "CNNRegression3Image"+road_name+"_graph.dat"
...
if len([file for file in os.listdir("Trained_NN") if file.__contains__("CNNRegression3Image"+road_name)]) > 0:
    train_saver.restore(sess, "Trained_NN/CNNRegression3Image"+road_name)
...
while iteration_count != 0:
    ...
    for iteration in range(iteration_count):
        ...
        if j < smallest_validation_error:
            ...
            train_saver.save(sess, "Trained_NN/CNNRegression3Image" + road_name)
