...
print "Direction Min : ", np.min(direction_data)
for img in data_dictionary.keys():
    data_dictionary[img][1] = (int(data_dictionary[img][1])+abs(int(np.min(direction_data))))
    data_dictionary[img][3] = (int(data_dictionary[img][3])-5)
data_file.close()
...
for dosya in validation_image_file_list:
    if dosya.__contains__("GRAYResized"):
        ...
        # we are adding image to dataset...
        ...
        validation_information_set.append(data_dictionary[dosya_number][1])# , int(data_dictionary[dosya_number][3])])
...
pickle.dump([direction_max, direction_min,
             ...
             whole_image_set.min(), diff], open("CNNClassificationTrainer("+road_name+")_important_data.dat", "w"))
...
error_graph_file_name = "CNNClassification"+road_name+"_graph.dat"
...
if len([file for file in os.listdir("Trained_NN") if file.__contains__("CNNClassification"+road_name)]) > 0:
...
train_saver.save(sess, "Trained_NN/CNNClassification"+road_name)
