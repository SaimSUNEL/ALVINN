...
direction_max, direction_min, direction_mean, direction_std, speed_max, speed_min, speed_mean, speed_std, mean_image, std_image, img_max, img_min, diff = \
    pickle.load(open("CNNRegression3ImageTrainer("+road_name+")_important_data.dat","r"))
    ...
    while True:
        ...
        if vehicle_image is not None:
            if image_count < 2:
                ...
                combination = np.reshape(combination, (100, 100, 3))
...
rospy.init_node("CNNRegression3ImageDriver", anonymous=False)
