...
for dosya in image_file_list:
    if dosya.__contains__("Resized") and dosya.__contains__("GRAY"):
        ...
        image_file = cv2.imread("../DataCollector/VehicleDataRoad3/" + str(image_number) + "GRAYGRAYResized.png")
...
pickle.dump([np.max(direction_data), np.min(direction_data),
             ...
             whole_image_set.min(), diff], open("RNNClassificationTrainer_important_data.dat", "w"))
...
camera_image = tf.placeholder(tf.float32, shape=[None, 100, 100])
...
w1 = tf.get_variable("w1", (50, 100), initializer=initializer)
...
init_range = math.sqrt(2.0 / input_number)
initializer = tf.random_uniform_initializer(-init_range, init_range)
w3 = tf.get_variable("w3", (100, 32), tf.float32, initializer)
b3 = tf.get_variable("b3", [32], tf.float32, tf.constant_initializer(0))

final_output = tf.nn.softmax(tf.matmul(fc_output, w3) + b3)

input_number = 25
init_range = math.sqrt(2.0 / input_number)
initializer = tf.random_uniform_initializer(-init_range, init_range)
W4 = tf.get_variable("W4", (100, 25), tf.float32, initializer)
b4 = tf.get_variable("b4", [25], tf.float32, tf.constant_initializer(0))

final_output2 = tf.nn.softmax((tf.matmul(fc_output, W4) + b4))
...
if len([file_ for file_ in os.listdir("Trained_NN") if file_.__contains__("RNNClassification")]) > 0:
    #train_saver.restore(sess, "Trained_NN/RNNClassification")
    sess.run(init)
...
train_saver.save(sess, "Trained_NN/RNNClassification")
