...
pickle.dump([np.max(direction_data), np.min(direction_data),
             ...
             whole_image_set.min(), diff], open("RNNClassificationRGB3ImageTrainer_important_data.dat", "w"))
print "max : ", whole_image_set.max()
print "min : ", whole_image_set.min()

print "Data have been loaded"

# We are packing three consequetive images as one image...
new_image_set = []
new_information_set = []
...
for image_index in range(2, len(image_set)):
    ...
    combinatination = np.reshape(combinatination, newshape=(9, 10000))
...
camera_image = tf.placeholder(tf.float32, shape=[None, 9, 10000])
...
if len([file_ for file_ in os.listdir("Trained_NN") if file_.__contains__("RNNClassificationRGB3Image")]) > 0:
    # train_saver.restore(sess, "Trained_NN/RNNClassificationRGB3Image")
    sess.run(init)
...
train_saver.save(sess, "Trained_NN/RNNClassificationRGB3Image")
