...
for dosya in validation_image_file_list:
    ...
    if dosya.__contains__("Resized") and dosya.__contains__("GRAY"):
...
image_set = [np.reshape(whole_image_set[index], (100*100))
             for index in range(0, whole_image_set.shape[0])]
validation_image_set = [np.reshape(validation_whole_image_set[index], (100*100)) for index in range(0, validation_whole_image_set.shape[0])]
...
pickle.dump([direction_max, direction_min,
             ...
             whole_image_set.min(), diff], open("HybridRegression3ImageTrainer("+road_name+")_important_data.dat", "w"))
...
error_graph_file_name = "HybridRegression3Image"+road_name+"_graph.dat"
...
if len([file for file in os.listdir("Trained_NN") if file.__contains__("HybridRegression3Image"+road_name)]) > 0:
    # Load previously saved model...

    train_saver.restore(sess, "Trained_NN/HybridRegression3Image"+road_name)
    ...
    print "smallest : " , smallest_validation_error
else:
...
while iteration_count != 0:
    ...
    for iteration in range(iteration_count):
        ...
        if j < smallest_validation_error:
            ...
            train_saver.save(sess, "Trained_NN/HybridRegression3Image" + road_name)
