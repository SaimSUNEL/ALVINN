...
road_name = "Road4"
direction_max, direction_min, direction_mean, direction_std, speed_max, speed_min, speed_mean, speed_std, mean_image, std_image, img_max, img_min, diff = pickle.load(open(root_directory+"MLPRegression3ImagesTrainer("+road_name+")_important_data.dat","r"))
...
for dosya in range(2, len(validation_image_file_list)):
    ...
    image_file3 = cv2.imread("../../DataCollector/"+location_name+"/"+str(dosya_number3)+"GRAYResized.png")
    image_file3 = image_file3[:, :, 0]

    combination = np.concatenate((np.reshape(image_file2, (1, camera_image_size**2)),
                                  np.reshape(image_file, (1, camera_image_size**2)),
                                  np.reshape(image_file3, (1, camera_image_size ** 2)))).reshape(30000)
...
train_saver.restore(sess, root_directory+"Trained_NN/MlpRegression3Images"+road_name)
...
dosya.write("MLPRegression3ImageTest %f" % j + "\n")
dosya.close()
